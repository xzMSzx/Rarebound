...
export function matchesMuseumCriteria(setId, cardId, criteria, setMap) {
  if (criteria.kind === 'set') return criteria.setIds.includes(setId);
  
  const mapToUse = setMap || getCachedSetCardsMap(setId);
  if (!mapToUse) return false;

  const apiCard = mapToUse.get(cardId);
...
export function getEligibleMuseumCards(criteria) {
  return runWithCollectionCache(() => {
    ...
    for (const setId of Object.keys(collection)) {
      const setMap = criteria.kind !== 'set' ? getCachedSetCardsMap(setId) : null;
