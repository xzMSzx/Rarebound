...
## 2024-05-24 - Accessibility for Unicode symbol buttons
**Learning:** Buttons relying on non-alphanumeric geometric Unicode characters (like `←` in `← Back`) may be poorly read by screen readers.
**Action:** Always add an explicit `aria-label` attribute (e.g. `aria-label="Go back"`) to elements containing confusing unicode symbols, as per the accessibility policy.
